...
export default function ModalForm({ opened, close, inititialForm, type, categories }: Props) {
      ...
      <Modal opened={opened} onClose={close} title={type === "ADD" ? "Add" : "Edit"}>
        <form onSubmit={form.onSubmit(async (values) => handleSubmit())}>
          <Stack>
            ...
            <TextInput type="number" required label="Stock" {...form.getInputProps("stock")} radius="md" min={1} />
